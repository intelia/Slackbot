...
function cleanProducts(raw) {
  ...
  for (const product of raw) {
    ...
    const sizes = (product.sizes || [])
      ...
      .map(s => ({ name: normalizeSizeName(s.name), id: s.id, price: s.price }));
...
function cleanCities(raw) {
  ...
  return namedZones.length > 0 ? { namedZones, rideHailTiers, pickupRows } : null;
}
...
async function loadFromAPI() {
  ...
  if (!products || !cities) {
    throw new Error(`API returned incomplete data (products: ${products?.length ?? 'null'}, cities namedZones: ${cities?.namedZones?.length ?? 'null'})`);
  }
...
async function init() {
  try {
    ...
    console.log(`[loader] Data refreshed from API — ${store.getProducts().length} products, ${store.getCities().namedZones.length} named zones`);
  } catch (err) {
    console.warn(`[loader] API fetch failed, using cached/static data: ${err.message}`);
    if (!lastLoadedAt) {
